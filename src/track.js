export class Track {
    ...
    update(player, level, isMobile) {
        ...
        if (this.spawnTimer <= 0 && this.enemies.length < maxEnemies) {
            this.spawnEnemies(player, level, isMobile); // Passando a verificação de mobile
    ...
    spawnEnemies(player, level, isMobile) {
        ...
        // REGRA DO MOBILE: Inimigos são SEMPRE mais lentos que você, logo, caem de cima pra baixo
        if (isMobile) {
            enemySpeed = Math.min(enemySpeed, Math.max(50, player.speed - 20));
        }

        // Define onde o carro vai nascer baseado em quem é mais rápido
